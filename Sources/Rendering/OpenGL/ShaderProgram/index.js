...
function substitute(source, search, replace, all) {
  ...
  // We don't need to instantiate a RegExp if we are working with a string and we
  // don't want to perform a global substitution
  const replaceSearch = typeof search === 'string' && all === false
    ? search
    // The same is true if we are working with a non string (a RegExp) and `all`
    // is set to `false`, here we can reuse the provided RegExp.
    : typeof search !== 'string' && all === false
    ? search
